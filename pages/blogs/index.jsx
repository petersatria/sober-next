...
const BlogList = (props) => {
	...
	const counDate = () => {
		...
		d.map(function (x) { counDate[x] = (counDate[x] || 0) + 1; });
	...
	const counCategory = () => {
		const countCat = {}
		const allCat = blogs.map(blog => blog.category)
		allCat.map(function (x) { countCat[x] = (countCat[x] || 0) + 1; });
				...
				<div className="col-12 col-lg-4 ps-5 my-5 d-none d-lg-block ">
					...
					<div className="my-5">
						...
						<div className="row text-center mt-4">
							{blogs && blogs.map(blog => {
								return <div key={blog.tag} className={`col mx-1 my-1 ${styles.tag}`}>{blog.tag[1]}</div>
							})}
						</div>
					</div>
				</div>

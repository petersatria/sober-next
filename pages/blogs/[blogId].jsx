...
const img = 'https://demo.uix.store/sober/wp-content/uploads/sites/2/2016/07/1-44.jpg'
		...
		<div className="container my-5">
			<div>
				<div className="row d-flex flex-row justify-content-center">
					<div className={`col-4 text-end ${styles.smallTitle}`} style={{ color: "#bfbfc8" }}>{dateConvert(date)}</div>
				...
				<div>
					<Image quality={30} width={1600} height={900} placeholder="blur" blurDataURL={"base64"} className={styles.blogImgDetail} src={blog.image} alt="" />
				</div>
				...
				<div className="my-5 d-flex justify-content-center">
					{blog.tag?.map(t => {
						return <div key={t} className={`mx-2 ${styles.tag}`}>{t}</div>
					})}
				</div>
			...
			<div className="row d-flex justify-content-center text-center">
				<div className="col-4">
					{blogs && shuffle(blogs).slice(3, 4).map(blog => (
						<Link key={blog._id} href={blog._id}>
							<p className={`${styles.pagePost} ${styles.cursorPointer}`}>PREVIOUS POST</p>
						</Link>
					))}
				</div>
				{/* FIXXXXX */}
				...
				<div className="col-4">
					{blogs && shuffle(blogs).slice(4, 5).map(blog => (
						<Link key={blog._id} href={blog._id}>
							<p className={`${styles.pagePost} ${styles.cursorPointer}`}>NEXT POST</p>
						</Link>
					))}
				</div>
			...
			<div>
				<div className="row d-flex flex-row justify-content-center text-center">
					...
					{relatedBlogs && relatedBlogs.slice(0, 3).map(blog => (
						<div key={Math.random().toString() + blog._id} className={`col-3`}>
...
export async function getServerSideProps(context) {
	...
	const { data } = await axios.get(`http://localhost:5000/api/blog/articles/${context.params.blogId}`)

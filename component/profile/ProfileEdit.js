...
function ProfileEdit(id) {
    ...
    const fetchdata = async (id) => {
        try {
          const { data } = await axios.get(`http://localhost:5000/api/profile/${id}`,config);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.post(`http://localhost:5000/api/profile/update/${id.id}`,objectData,config)
  ...
  const handlePassword = async (e) => {
    ...
    if (newPassword !== cPassword) {
      ...
      try {
        ...
        const response = await axios.post(`http://localhost:5000/api/profile/settings/${id.id}`, datapw, config)
    ...
    <div className='row'>
        ...
        <div className="card bg-light  border-0">
          ...
          <hr/>
            <div className='row'>
              {notif ? 
                <Alert variant='dark'>success update profile</Alert> : ""
            ...
            <h4 className={style.heading}>Change Password</h4>
              {
                pwNotif ? <Alert variant='dark'>successfully change password</Alert> : ""
              }
              {
